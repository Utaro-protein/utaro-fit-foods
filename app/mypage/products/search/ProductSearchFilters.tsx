...
function snapTen(v: number, min: number, max: number): number {
...
function snapPair(
  ...
  const a = snapTen(lo, min, max);
  const b = snapTen(hi, min, max);
...
function DualRangeRow({
  ...
  function handleChange(next: number[]) {
    ...
    const rawA = Math.min(Math.max(next[0], min), max);
    const rawB = Math.min(Math.max(next[1], min), max);
    onChange(snapPair(rawA, rawB, min, max));
  }
...
export function ProductSearchFilters({ bounds, ranges, onAfterNavigate }: Props) {
  ...
  function reset() {
    const resetRanges = snapRangeState(
      {
